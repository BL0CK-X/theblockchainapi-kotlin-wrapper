...
> kotlin.collections.List&lt;kotlin.Any&gt; solanaGetTokensBelongingToWallet(network, publicKey)
...
try {
    val result : kotlin.collections.List<kotlin.Any> = apiInstance.solanaGetTokensBelongingToWallet(network, publicKey)
    println(result)
} catch (e: ClientException) {
